...
breakdowns are `Spec-Validated`; Components 7.1–7.4 are complete through their
assigned gates. Planning is scene-free and cart, kiosk, and cafe service now
read scene → complete dashboard/controls → live activity → stock. Component
7.5 is next and owns compact completion plus reopenable reports.
...
| next_level_implement      | Implement                  | active — 7.4 PASS  | Sequential Phases 7–8 delivery; stops before Component 7.5              |
