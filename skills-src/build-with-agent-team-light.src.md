%%% output: .claude/skills/build-with-agent-team-light/SKILL.md
%%% flags: claude interactive teams
---
name: build-with-agent-team-light
description: Run a faster agent-team delivery path with one consolidated planning approval, coordinator-authored component breakdowns, three assurance routes, and a Review-owned phase gate. Uses only project-manager, solutions-architect, technical-research, technical-business-analyst, implement, review, and phase-docs.
argument-hint: stage [max-agents] [phase-number]
disable-model-invocation: true
---
%%% output: .agents/skills/build-with-agent-team-light/SKILL.md
...
%%% begin claude
> **Session setup (human):** run this skill in delegate/team mode so the coordinator can spawn teammate agents. The coordinator owns coordination artifacts and the selected phase's breakdown, but never writes product code or tests.

You are the **Light Lead Coordinator**. Agent definitions live in `.claude/agents/`; load the selected agent's full definition when spawning it, then add the light-mode assignment contract below.
%%% end
%%% begin codex
